...
export default {
  ...
  state: {
    serverChatEn: {
      ...
      serverChatName: '小P',
      avatarUrl: '/static/img/im_server_avatar.png'
    },
  ...
  mutations: {
     ...
     * @param state
     */
    setIoSocket: function (state) {
      console.log('set io socket')
      state.socket = ioClient(`${window.SITE_CONFIG['ws']}`, { path: `/renren-fast/im/websocket/platform/${Vue.cookie.get('token')}` })
    }
  },
  actions: {
    createWs ({commit}, ws) {
    ...
    getChatEnByChatId: function (context, { clientChatId, listName }) {
      ...
      if (listName) {
        ...
        // 2.未指定列表
        // 1)从当前会话列表查找
        for (var i1 = 0; i1 < context.state.currentChatEnlist.length; i1++) {
          var tmpEn1 = context.state.currentChatEnlist[i1]
          if (tmpEn1.clientChatId === clientChatId) {
            chatEn = tmpEn1
            break
          }
        }
    ...
    addNotificationChat: function (context, { chatEn, oprType }) {
      ...
      // 7.显示通知
      for (var i1 = 0; i1 < state.notificationChatEnlist.length; i1++) {
        const item = state.notificationChatEnlist[i1]
        ...
        notification.onclose = function (e) {
          ...
          // remove notification
          for (var t = 0; t < window.imServerStore_notificationList.length; t++) {
            if (window.imServerStore_notificationList[t].tag === notification.tag) {
              window.imServerStore_notificationList.splice(t, 1)
              break
            }
          }
        }
     ...
     * 服务端上线
     */
    SERVER_ON: async function (context, payload) {
      await context.commit('setIoSocket')
      const serverChatEn = {
      }
