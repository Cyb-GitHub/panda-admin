    ...
    <div class="common_chat-inner">
      ...
      <div v-if="chatLoaded" class="common_chat-main" id="common_chat_main" ref="common_chat_main">
        <div class="common_chat-main-content">
          <div class="inner">
            <div class="item sys">
              <div class="text-content" @click="showMoreMsg">
                查看更多
              </div>
            </div>
            <div v-for="(item ,index) in msgList" :key="index">
              <!-- 客户、客服 -->
              <div class="item" :class="{ sender: item.source, receiver: item.source === 0 }">
                <div class="info-wrapper" :class="item.state">
                  ...
                  <!-- 1)文本类型 -->
                  <div v-if="item.msgType==1 || item.type==1" class="item-content common_chat_emoji-wrapper-global">
                    <p class="text" v-html="getqqemojiEmoji(item.content)"></p>
                  </div>
                  <!-- 2)图片类型 -->
                  <div v-else-if="item.msgType === 2  || item.type==2" class="item-content">
      ...
      <div class="common_chat-footer">
        <div>
          ...
          <!-- 发送按钮 -->
          <el-button type="primary" size="small" class="send-btn" :class="chatInfoEn.state" @click="sendText()" :disabled="inputContent.length==0">发送</el-button>
        </div>
        <!-- 离线 -->
...
export default {
  ...
  methods: {
      ...
      if (this.chatInfoEn.state === 'on') {
        ...
        setTimeout(function () {
          ...
          if (self.inputContent) {
            self.setInputDiv(self.inputContent)
          }
        }, 200)
      } else {
    ...
    sendText: function () {
      var self = this
      if (self.inputContent.length === '') {
        return
      }
      var msgContent = self.inputContent
      document.getElementById('common_chat_input').innerHTML = ''
