      ...
      <div v-if="storeCurrentChatEnlist.length>0" class="item-list">
        <div class="item" v-for="(tmpEn, index) in storeCurrentChatEnlist" :key="index" @click="selectChat(tmpEn)" v-bind:class="{ active: selectedChatEn!=null && tmpEn.clientChatId==selectedChatEn.clientChatId}">
          ...
          <div class="info-wrapper">
            <p class="first-p">
              <span class="name">{{tmpEn.userId}}</span>
              <span class="lastMsgTime">{{getLastMsgTimeStr(tmpEn.latestMsgTime)}}</span>
            </p>
            <p class="second-p">
              <span class="lastMsgContent" v-html="tmpEn.latestMsg"></span>
              <el-badge class="newMsgCount" :value="tmpEn.unread" :max="99" v-show="tmpEn.unread>0"></el-badge>
            </p>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    selectedChatEn () {
      return this.$store.getters.selectedChatEn
    },
    // // 当前会话列表
    // storeCurrentChatEnlist () {
    //   return this.$store.getters.currentChatEnlist
    // },
    storeServerChatEn () {
  ...
  methods: {
    getConversations () {
      conversations().then(res => {
        console.log(res)
        if (res.data.records) {
          this.storeCurrentChatEnlist = res.data.records
        }
      })
    },
    /**
    ...
    selectChat: function (en) {
      this.$emit('selectedChat', en) // 事件上传
    },
    ...
    toggleFollowIcon: function (en) {
      ...
      // 排序
      this.$store.commit('sortCurrentChatEnlist', {})
    },
